   ...
   methods: {
      ...
      changeFavour(messageID, indexOut, indexIn) {
         if (indexIn === undefined) {
            ...
            this.$axios
               .post("api/message/update", this.$qs.stringify({
                  messageID:messageID,
                  favour:this.messageList[indexOut].favour+(this.messageList[indexOut].isFavour ? 1:0)
               }))
               .then((response) => {
                  console.log("添加结果", response);
                  if(response.data.code){
                     this.messageList[indexOut].favour+=1
                  }
