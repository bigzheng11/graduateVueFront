         ...
         <li v-for="item in deliveryAddressArr" :key="item.daId">
            <div class="addresslist-left" @click="setDeliveryAddress(item)">
               <h3>
                  ...
                  <div>{{ item.provinceCity }}{{ item.addressDetail }}</div>
               </h3>
               <p>{{ item.address }}</p>
            </div>
            <div class="addresslist-right">
               ...
               ></i>
               <i class="el-icon-edit" @click="editUserAddress(item)"></i>
         ...
         <li>
            <div class="addresslist-left">
               <h3 style="margin-top:10px;color: black;">
                     {{ addressNow==""?"请点击定位按钮进行定位":addressNow}}
               </h3>
            </div>
            <div class="addresslist-right">
               <el-button type="primary" plain style="padding:9px" @click="toUserCenter">使用此位置</el-button></span>
            </div>
         </li>
      </ul>
...
export default {
   ...
   methods: {
      ...
      locationSuccess(point, AddressComponent, marker) {
         let address = point.addressComponent;
         console.log("定位成功");
         this.addressNow =
      ...
      listDeliveryAddressByUserId() {
         this.$axios
            ...
            .then((response) => {
               this.deliveryAddressArr = response.data.data.addressList;
               console.log("地址:", this.deliveryAddressArr);
      ...
      setDeliveryAddress(deliveryAddress) {
         ...
         //从session中取出
         let userinfo = this.$qs.parse(
            window.sessionStorage.getItem("userinfo")
         );

         userinfo.addressDefault =
            deliveryAddress.provinceCity + deliveryAddress.addressDetail;
      ...
      toUserCenter() {
         if (this.addressNow == "") {
            ...
            userinfo.addressDefault = this.addressNow;
