<template>
   <div class="container">
      ...
      <div class="leaveMessage">
         ...
         <div v-for="(item, indexOut) in messageList" :key="item.messageID">
            ...
            >
               ...
               <span
                  style="position: absolute; left: 22%; top: 42%"
                  @click="showPopup(itemTwo.messageID, item.messageID)"
               >
                  {{ itemTwo.message }}
               </span>
               <!-- 点赞 -->
      ...
      >
         <!-- 收藏 -->
         <span style="position: relative; right: 10%; font-size: 14px">
            <div>
               <van-icon
         ...
         <span>
            <van-icon name="comment-o" size="26" />
            <p @click="showPopup" style="font-size: 14px">添加留言</p>

            <van-popup v-model="show" position="top" style="margin-top: 100px">
               <el-input
                  type="textarea"
                  :rows="2"
                  placeholder="请输入内容"
...
export default {
   ...
   created() {
      ...
      // this.goodsDetail = this.$qs.parse(
      //    window.sessionStorage.getItem("nowGoodItem")
      // );
      this.$axios
         .get("api/goods/getById/" + this.goodsID)
         .then((response) => {
            this.goodsDetail = response.data;
            //留言列表遍历
            for (const item of this.goodsDetail.messagesList) {
               if (item.toMessageID == null || item.toMessageID == 0) {
   ...
   methods: {
      //弹出层
      showPopup(toMessageID, parentTop) {
         this.show = true;
         if (typeof toMessageID != "object") {
            if (parentTop != undefined) {
               this.parentTop = parentTop;
               this.presentToMessageID = toMessageID;
               return;
            } else {
               this.parentTop = toMessageID;
               this.presentToMessageID = toMessageID;
               return;
            }
         } else {
            this.parentTop = 0;
            this.presentToMessageID = 0;
            return;
         }
      },
      // 发送留言
      ...
      toBuyPage() {
         ...
         // 删除原商品
         this.$axios
            .get("api/goods/backstageDeleteById/", this.goodsID)
            .then((response) => {
               console.log("删除信息",response.data);
            })
            .catch((error) => {
               console.log(error);
            });
         ...
         //添加历史订单
         this.$axios
            .post("api/order/add",  this.$qs.stringify({
                     userID: this.user.userID,
                     goodsID: this.goodsDetail.goodsID,
                     isCollect: !this.isCollect ? "1" : "0",
                  }))
